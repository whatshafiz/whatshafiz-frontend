...
onBeforeMount(async () => {
  ...
  const universityId = route.query.universityId
...
const onSubmit = async () => {
  ...
  if (await universityStore.createUniversityFaculty(faculty.university_id, faculty.name)) {
    isLoading.value = false
    successNotificationToggle('İşlem Başarılı', 'Yeni Fakülte Oluşturuldu.')
...
<template>
  <div v-if="user.can('universities.update')">
    <div class="flex items-center mt-8 intro-y">
      <h2 class="mr-auto text-lg font-medium">Yeni Fakülte Oluştur</h2>
    </div>
