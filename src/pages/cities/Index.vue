  ...
  },
  {
    ...
    formatter(cell) {
      ...
      return `<div>
                ...
                <div class="text-xs text-slate-500 whitespace-nowrap">ÜLKE: ${response.country.name}</div>
              </div>`;
    },
  },
  {
  ...
  },
  {
    ...
    formatter(cell) {
      ...
      editButton.addEventListener("click", function () {
        router.push({ name: 'cities.edit', params: { cityId: rowData.id } })
      });
      deleteButton.addEventListener("click", function (event) {
        alertStore.setDeleteModalPreview(true)
        alertStore.setDeleteModalAction(() => deleteCity(rowData.id))
      });
...
<template>
  <div v-if="user.can('countries.update')">
    <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
      <h2 class="mr-auto text-lg font-medium">Tüm Şehirler</h2>
        <div class="flex w-full mt-4 sm:w-auto sm:mt-0">
          <RouterLink :to="{ name: 'cities.create' }">
    ...
    <datatable ref="tableRef" :index-url="countryStore.getCitiesIndexURL" :columns="tableColumns" />
  </div>
</template>
