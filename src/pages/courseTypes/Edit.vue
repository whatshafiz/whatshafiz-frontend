...
const courseTypeId = route.params.courseTypeId
...
onBeforeMount(async () => {
  ...
  courseType.value = await courseTypeStore.fetchCourseType(courseTypeId)
})
...
const onSubmit = async () => {
  ...
  if (await courseTypeStore.updateCourseType(courseTypeId, courseType.value)) {
    isLoading.value = false
    successNotificationToggle('İşlem Başarılı', 'Kurs Türü Bilgileri Güncellendi.')
...
<template>
  <div v-if="user.can('courseTypes.update')">
    <div class="flex items-center mt-8 intro-y">
      <h2 class="mr-auto text-lg font-medium">Kurs Türü Bilgilerini Düzenle</h2>
    </div>
