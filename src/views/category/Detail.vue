<template>
  ...
  <div class="pos intro-y grid grid-cols-12 gap-5 mt-5">
    ...
    <div class="col-span-12 lg:col-span-4">
      ...
      <div class="intro-y box p-5">
        <div class="form-check form-switch flex flex-col items-start">
          <button class="btn btn-primary w-full" :disabled="validate.catInfo.$invalid" @click="handleUpdate">Güncelle</button>
        </div>
        <div class="form-check form-switch flex flex-col items-start mt-5">
          <button class="btn btn-danger w-full" :disabled="state.hasChild"  @click="state.previewDialogModal = true">Sil</button>
          <p class="text-xs mt-1" v-show="state.hasChild">Bu kategori alt kategoriler içerdiğinden silinemez.</p>
        </div>
      </div>
    </div>
  </div>
...
const getCategory = async () => {
  axios.get('/admin/category/'+props.id).then((response) => {
    let resp = response.data.data;
    catInfo.name = resp.name;
    catInfo.meta_description = resp.meta_description;
    catInfo.meta_title = resp.meta_title;
    catInfo.parent_id = resp.parent_id == 0 ? null : resp.parent_id;
    catInfo.active = resp.active;
    catInfo.type = resp.type;
    if(resp.getchilds.length > 0){
      state.hasChild = true;
    }
...
const handleUpdate = () => {
  ...
  axios.post('/admin/category/'+props.id,catInfo).then((response) => {
...
const handleRemove = () => {
  axios.delete('/admin/category/'+props.id).then((response) => {
